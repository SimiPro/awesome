package com.simi.awesome.utils.marshalling;
...
public class JaxbMarshalUnmarshalUtil {
	...
	public static <T> T unmarshal(String xsdSchema, String xmlDatei, Class<T> clss) throws JAXBException, SAXException {
	...
	public static <T> T unmarschalSchemaUrl(String schemaUrl, String xmlData, Class<T> clazz) throws IOException,
			SAXException, JAXBException {
		SchemaFactory schemaFactory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
		Schema schema = (schemaUrl == null || schemaUrl.trim().length() == 0) ? null : schemaFactory.newSchema(new URL(
				schemaUrl));
		JAXBContext jaxbContext = JAXBContext.newInstance(clazz);
		return unmarshal(jaxbContext, schema, xmlData, clazz);
	}
	...
	public static void marshal(String xsdSchema, String xmlDatei, Object jaxbElement) throws JAXBException,
			SAXException {
		SchemaFactory schemaFactory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
		Schema schema = (xsdSchema == null || xsdSchema.trim().length() == 0) ? null : schemaFactory
				.newSchema(new File(xsdSchema));
